...
rename_pars(pars_long) %>%
    ...
    facet_wrap(~ parname, ncol = 3) + get_theme(txt = 11, legend.position = c(0.85, 0.25)) +
...
Plots and results for main text
...
# To quote total antivirals used in main text, choose one effectiveness scenario (they're all the same)
...
p_total <- total_averted %>% 
    ...
    labs(x = "Antiviral effectiveness", y = "Averted hospitalizations relative to worst-case (#)")
...
change.worst %>% filter(age != "all") %>%
    ...
    labs(x = "Age (years)", y = "Averted hospitalizations relative to worst-case (#)")
...
# NNT
nai.used.age.risk <- out.baseline %>% 
...
pars.alt.cs <- bind_rows(tmp.pars, pars.alt.cs1, pars.alt.cs2, pars.alt.cs3, pars.alt.cs4) %>% 
                mutate(alternative = rep(c("baseline", "more care-seeking", 
                                           "more early-care-seeking", "more compliance",
                                           "more everything"), each = nrow(tmp.pars))) %>%
...
pars_long <- 
    ...
    filter( alternative == "baseline" | (param == "seek.care" & alternative ==  "more care-seeking") | 
            (param == "seek.care.early" & alternative ==  "more early-care-seeking") | 
                (param == "nai.compliance" & alternative ==  "more compliance")) %>%
    mutate(scenario = ifelse(alternative == "baseline", alternative, "50% increase"))
...
ggsave(here("figs", "figS1.png"), width = 8, height = 6)
```
...
p_alt_num <- 
    ...
    facet_grid(~ age) + get_theme(txt = 9, legend.position = "top") + 
    scale_y_log10(labels = comma) +
    scale_color_manual(NULL, values = cols_alt) +
    labs(x = "Risk stratification", y = "Averted hospitalizations relative to worst-case (#)")
...
p_alt_perc <- 
    ...
    geom_errorbar(aes(ymin = lower, ymax = upper), 
                  position = position_dodge(width = dodgewdth + 0.1), width = barsize + 0.1, linewidth = barwdth) +
    facet_grid(~ age) + get_theme(txt = 9) + 
    scale_y_continuous(labels = comma) + 
    scale_color_manual(guide = "none", values = cols_alt) +
    labs(x = "Risk stratification", y = "Averted hospitalizations relative to worst-case (%)")

p_alt_num / p_alt_perc + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1, 0.9))
ggsave(here("figs", "fig4.png"), width = 8, height = 7.5)
```
...
pars.alt <- bind_rows(tmp.pars, pars.alt1, pars.alt2, pars.alt3) %>% 
                mutate(alternative = rep(c("baseline", 
                                           "prescribing",
                                           "testing",
                                           "testing & prescribing"), each = nrow(tmp.pars)))
...
p_alt2_num <- 
    ...
    facet_grid(~ age) + get_theme(txt = 9, legend.position = "top") + 
    scale_y_log10(labels = comma) +
    scale_color_manual("Scenario", values = cols_alt) +
...
p_alt2_num / p_alt2_perc + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1, 0.9))
ggsave(here("figs", "figS5.png"), width = 8, height = 7.5)
```
...
p_alt3_num / p_alt3_perc + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(1, 0.9))
ggsave(here("figs", "figS6.png"), width = 8, height = 7.5)
```

## Everything single and then combined
...
pars.alt <- bind_rows(tmp.pars, pars.alt1, pars.alt2, 
                      pars.alt.cs1, pars.alt.cs2, pars.alt.cs3, 
                      pars.all) %>% 
                mutate(alternative = rep(c("baseline", 
                                           "prescribing",
                                           "testing",
                                           "care-seeking", 
                                           "early-care-seeking", 
                                           "compliance", "all parameters"), each = nrow(tmp.pars))) %>%
    # make sure new inputs don't exceed 1
    mutate(across(c(seek.care, seek.care.early, nai.compliance), function(x) pmin(x, 1)))
```
