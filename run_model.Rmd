...
# optional figure showing estimated # antivirals used
out.baseline %>% group_by(sim, testing, prescribing, effectiveness) %>%
    summarize(nai.total = sum(nai.total), .groups = "drop") %>%
    # choose one effectiveness scenario (they're all the same)
    filter(effectiveness == "high NAI effect") %>%
    do(., get_summary(., var = "nai.total", groups = c("testing", "prescribing"))) %>%
    ggplot(aes(x = testing, y = mean, color = prescribing)) + 
    geom_point(size = ptsize, position = position_dodge(0.4)) +
    geom_errorbar(aes(ymin = lower, ymax = upper), width = barwdth/3, position = position_dodge(0.4), #alpha = 0.75,
                 linewidth = barsize) +
...
total_averted %>% 
    ...
    get_theme(txt = 9, legend.position = "top") + 
...
ggsave(here("figs", "figS2.png"), width = 6, height = 5, dpi = 800)
